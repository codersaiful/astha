...
if ( ! function_exists( 'astha_posted_on' ) ) :
	...
	function astha_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( '%s', 'post date', 'astha' ),// phpcs:ignore WordPress.WP.I18n.NoEmptyStrings
...
if ( ! function_exists( 'astha_get_category' ) ) :
	/**
	 * Prints HTML with meta information for the current author.
	 */
	function astha_get_category() {
		$category = get_the_category();
		if ( !empty($category) && $category[0] ){?>
			<span>
			<i class="fa fa-bookmark-o"></i>
			<a  class="astha-cat" href="<?php echo esc_url(get_category_link( $category[0]->term_id ));?>">
	...
	function astha_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( 'Posted in %1$s', 'astha' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( 'Tagged %1$s', 'astha' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
...
if ( ! function_exists( 'astha_post_thumbnail' ) ) :
	...
	function astha_post_thumbnail() {
		...
		<a class="post-thumbnail lazy-load replace" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
			<?php
			the_post_thumbnail( 'astha-blog-thumbnail', array(
